...
export const Sound = {
    ...
    shoot: (type) => {
        if(!actx) return;
        if(type === 'bullet') { Sound.triggerNoise(0.1, 0.2, 3000); Sound.triggerTone(800, 'square', 0.1, 0.1, 100); }
        else if(type === 'spread') { Sound.triggerTone(60, 'sine', 0.4, 0.5, 10); Sound.triggerNoise(0.3, 0.5, 1000); Sound.triggerNoise(0.1, 0.3, 5000); }
        else if(type === 'chaingun') { Sound.triggerNoise(0.08, 0.3, 2500); Sound.triggerTone(300, 'sawtooth', 0.05, 0.1, 50); }
        else if(type === 'rocket') { Sound.triggerTone(150, 'triangle', 0.5, 0.4, 50); }
        else if(type === 'bfg') { Sound.triggerTone(50, 'sawtooth', 1.2, 0.5, 800); }
    },
    explode: () => { Sound.triggerNoise(0.8, 0.8, 800); Sound.triggerTone(80, 'triangle', 1.0, 0.6, 10); },
    hit: () => { Sound.triggerNoise(0.1, 0.1, 3000); Sound.triggerTone(150, 'sawtooth', 0.05, 0.1, 50); },
